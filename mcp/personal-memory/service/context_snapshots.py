...
def _latest_topic_snapshot(*, user_code: str, session_key: str, topic_key: str) -> Optional[Dict[str, Any]]:
    with get_conn() as conn, conn.cursor() as cur:
        cur.execute(
            ...
            WHERE user_code = %s
              AND session_key = %s
              AND snapshot_level = %s
              AND topic_key = %s
              AND status = %s
            ORDER BY updated_at DESC, id DESC
            LIMIT 1
            """,
            (user_code, session_key, SNAPSHOT_TOPIC, topic_key, STATUS_ACTIVE),
        )
        row = cur.fetchone()
...
def _latest_global_topic_snapshot(*, user_code: str, topic_key: str) -> Optional[Dict[str, Any]]:
    with get_conn() as conn, conn.cursor() as cur:
        cur.execute(
            ...
            LIMIT 1
            """,
            (user_code, SNAPSHOT_GLOBAL_TOPIC, topic_key, STATUS_ACTIVE),
        )
        row = cur.fetchone()
        return dict(row) if row else None
...
def _persist_session_memory_from_snapshot(
    *,
    ...
    for item in analysis_results:
        ...
        if action != ACTION_LONG_TERM or not claim:
            continue
    ...
    *,
    ...
    if turns:
        for turn in turns:
            ...
            if role not in {ROLE_USER, ROLE_ASSISTANT} or not content:
                continue
    ...
    segment_row = _save_snapshot(
        {
            "user_code": resolved_user,
            "session_key": session_key,
            "snapshot_level": SNAPSHOT_SEGMENT,
            "topic_key": segment["topic_key"],
            "topic": segment["topic"],
            "summary": segment["summary"],
            "user_view": segment.get("user_view"),
    ...
    global_payload = {
        ...
        "snapshot_level": SNAPSHOT_GLOBAL_TOPIC,
...
def search_recent_context_summaries(
    *,
    user_code: Optional[str] = None,
    session_key: Optional[str] = None,
    query: str = "",
    ...
    if session_key:
        ...
        params.append(session_key)
    if levels:
        conditions.append("snapshot_level = ANY(%s)")
        params.append(levels)
    if recent_hours > 0:
        conditions.append("coalesce(ended_at, updated_at, created_at) >= now() - (%s * interval '1 hour')")
        params.append(int(recent_hours))
    if query.strip():
        tokens = [token for token in re.split(r"\s+", query.strip()) if token]
        for token in tokens:
            like = "%" + token + "%"
